...
# Description: Triggers the GitHub Actions CI/CD pipeline by pushing an empty commit
# to the main branch. Supports both automatic semantic incrementing and explicit
...
# ------------------------------------------------------------------------------
# If an argument is provided (e.g., ./trigger-release.sh v1.2.0), use it as manual
# override. Otherwise, create a technical standard commit to let the pipeline handle
# automatic incrementing (+1 patch).
if [ $# -gt 0 ]; then
  VERSION_INPUT="$1"

  # Enforce standard formatting for safety (ensures input starts with v followed
  # by a number)
