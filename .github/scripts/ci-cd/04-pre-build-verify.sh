...
# Using a temporary file descriptor array to prevent pipeline variable isolation
# issues
...
while read -r modfile; do
  ...
  # Subshell deployment ensures environment isolation without breaking sequential
  # loops
  (
    cd "$target_dir"
    go build ./...
