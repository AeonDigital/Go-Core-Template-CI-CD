...
Every Job begins by executing `00-bootstrap-workspace.sh` to copy the shared script engines from the central repository directly into the running virtual environment container.
...
*   **Core Responsibilities:**
    ...
    *   Verifies Go source formatting using `02-verify-go-formatting.sh` before moving on to the remaining validation steps.
    ...
    *   **Multi-Platform Matrix Testing:** Runs the full unit test suite via `03-run-tests.ps1` concurrently across three distinct operating systems: **Linux (Ubuntu), Windows, and macOS**, centralizing the coverage profile inside `.github/coverage.out`.
...
*   **Core Responsibilities:**
    *   Executes `04-pre-build-verify.sh` to perform an integrity verification compilation (`go build`) across all local modules recursively. This acts as a critical safety gate before any deployment or version marking occurs.
...
*   **Core Responsibilities:**
    *   **Semantic Tagging:** Executes `06-manage-version-tag.sh` to calculate the next Semantic Versioning (SemVer) target based on commit history and publishes the new immutable version tag (`vX.Y.Z`) upstream.
    *   **Conditional Artifact Compilation:** Executes `05-check-entrypoints.sh` to inspect `.github/entrypoints.txt`. If valid active paths are found, it triggers GoReleaser using the static configuration file (`-f .github/release/config.yaml`) to cross-compile binary executables and publish a formal **GitHub Release** populated with distribution artifacts. If the file is empty or contains only comments, it safely skips binary compilation, treating the repository strictly as a library.
